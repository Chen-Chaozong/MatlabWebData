...
%% 
    ...
    try
        ...
        switch type
            ...
            case 'operation'
                cwfx.yszkzzl = data.web.webstr2num(cwfx.yszkzzl);  % 应收账款周转率
                cwfx.yszkzzts = data.web.webstr2num(cwfx.yszkzzts); % 应收账款周转天数
                cwfx.chzzl = data.web.webstr2num(cwfx.chzzl);  % 存货周转率
                cwfx.chzzts = data.web.webstr2num(cwfx.chzzts);  % 存货周转天数
                cwfx.ldzczzl = data.web.webstr2num(cwfx.ldzczzl);  % 流动资产周转率
                cwfx.ldzczzts = data.web.webstr2num(cwfx.ldzczzts);  % 流动资产周转天数
            case 'grow'
                cwfx.mainincome = data.web.webstr2num(cwfx.mainincome);  % 主营业务收入增长率
                cwfx.netprofit = data.web.webstr2num(cwfx.netprofit); % 净利润增长率
                cwfx.netassets = data.web.webstr2num(cwfx.netassets);  % 净资产增长率
                cwfx.totalassets = data.web.webstr2num(cwfx.totalassets);  % 总资产增长率
                cwfx.eps = data.web.webstr2num(cwfx.eps);  % 每股收益增长率
                cwfx.sgr = data.web.webstr2num(cwfx.sgr);  % 股东权益增长率
            case 'debtpaying'
                cwfx.current = data.web.webstr2num(cwfx.current);  % 流动比率
                cwfx.quick = data.web.webstr2num(cwfx.quick); % 速动比率
                cwfx.cash = data.web.webstr2num(cwfx.cash);  % 现金比率
                cwfx.icr = data.web.webstr2num(cwfx.icr);  % 利息支付倍数
                cwfx.equity = data.web.webstr2num(cwfx.equity);  % 股东权益比率
                cwfx.debt2asset = data.web.webstr2num(cwfx.debt2asset);  % 资产负债率
            case 'cashflow'            
                cwfx.cashflow2income = data.web.webstr2num(cwfx.cashflow2income);  % 经营现金净流量对销售收入比率
                cwfx.cashflowrepay = data.web.webstr2num(cwfx.cashflowrepay); % 资产的经营现金流量回报率
                cwfx.cashflow2netprofit = data.web.webstr2num(cwfx.cashflow2netprofit);  % 经营现金净流量与净利润的比率
                cwfx.cashflow2debt = data.web.webstr2num(cwfx.cashflow2debt);  % 经营现金净流量对负债的比率
                cwfx.cashflow = data.web.webstr2num(cwfx.cashflow);  % 现金流量比率
            case 'mainindex'
                cwfx.eps = data.web.webstr2num(cwfx.eps);  % 每股收益
                cwfx.epsrate = data.web.webstr2num(cwfx.epsrate); % 每股收益同比
                cwfx.navps = data.web.webstr2num(cwfx.navps);  % 每股净资产
                cwfx.roe = data.web.webstr2num(cwfx.roe);  % 经营现金净流量对负债的比率
                cwfx.cps = data.web.webstr2num(cwfx.cps);  % 现金流量比率
                cwfx.netprofit = data.web.webstr2num(cwfx.netprofit);  % 经营现金净流量对负债的比率
                cwfx.netprofitrate = data.web.webstr2num(cwfx.netprofitrate);  % 现金流量比率
            case 'performance'
                cwfx.pre_eps = data.web.webstr2num(cwfx.pre_eps);  % 上年同期每股收益
            case 'news'            
                cwfx.eps = data.web.webstr2num(cwfx.eps);  % 每股收益
                cwfx.yeps = data.web.webstr2num(cwfx.yeps); % 营业收入（万元）
                cwfx.epsrate = data.web.webstr2num(cwfx.epsrate);  % 营业收入同比
                cwfx.netprofit = data.web.webstr2num(cwfx.netprofit);  % 净利润（万元）
                cwfx.netprofitrate = data.web.webstr2num(cwfx.netprofitrate);  % 净利润同比
                cwfx.navps = data.web.webstr2num(cwfx.navps);  % 每股净资产（元）
                cwfx.roe = data.web.webstr2num(cwfx.roe);  % 净资产收益率
                cwfx.sumnavps = data.web.webstr2num(cwfx.sumnavps);  % 总资产（万元）
            case 'incomedetail'
                cwfx.income = data.web.webstr2num(cwfx.income);  % 每股收益
                cwfx.cost = data.web.webstr2num(cwfx.cost); % 营业收入（万元）
                cwfx.profit = data.web.webstr2num(cwfx.profit);  % 营业收入同比
        end
